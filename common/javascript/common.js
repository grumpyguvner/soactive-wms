		...
		// branch for IE/Windows ActiveX version
			...
			if (req) {
				...
				req.send();
                    ...
                    alert("Your browser does not support Server calls, please try a different browser");
                }
...
function modalAlert(text, myTitle){
	...
	text.replace("\n","<br />");
        ...
        $('#dialog').dialog({
			modal: true,
			title: (myTitle = (typeof myTitle == "undefined")?'Attention:':myTitle),
			buttons: {
				Ok: function() {
					$( this ).dialog( "close" );
				}
