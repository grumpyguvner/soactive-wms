...
class bleep_db{
    ...
    function bleep_db($connect = true, $hostname = NULL, $schema = NULL, $user = NULL, $pass = NULL, $pconnect = NULL, $type = "mysql"){
        if($type!="mysql")
        $this->type=$type;
        ...
        switch($this->type){
            ...
            case "mysql":
                ...
                if(defined("BLEEP_HOSTNAME"))
                    $this->hostname = BLEEP_HOSTNAME;

                if($hostname!=NULL)
                    $this->hostname = $hostname;

                if(defined("BLEEP_DATABASE"))
                    $this->schema = BLEEP_DATABASE;

                if($schema!=NULL)
                    $this->schema = $schema;

                if(defined("BLEEP_USER"))
                    $this->dbuser = BLEEP_USER;

                if($schema!=NULL)
                    $this->dbuser = $user;

                if(defined("BLEEP_PASSWORD"))
                    $this->dbpass = BLEEP_PASSWORD;

                if($schema!=NULL)
                    $this->dbpass = $pass;

                if(defined("BLEEP_PCONNECT"))
                    $this->pconnect = BLEEP_PCONNECT;
