...
class importBleepStyles{

	var $db;
	var $bleep_db;

	function importBleepStyles($db){
		$this->db = $db;
	...
	function importBleepStyleRecords(){

//		$logError = new appError(0, "Error Details", "Bleep Import Styles"); //also stops execution!
...
//		$logError = new appError(0, "NEED TO DEACTIVE NON WEB ENABLED STYLES", "Bleep Import Styles"); //also stops execution!
...
//                if(!$queryresult){
//                        $error= new appError(0,"Could Not Update Import Date","Bleep Import Styles");
//                }
...
//                    $throttle = BLEEP_STYLETHROTTLE;
		...
		$bleep_statement = "SELECT
					ALSTYLES.id AS id,
					ALSTYLES.description AS stylename,
					...
					ALSTYLES.current_stock AS bleep_stock,
					CONCAT(ALSTYLES.department,'_',ALSTYLES.user_string_1) AS alternate_depts,
					froogle.description AS webdescription
					FROM ALSTYLES 
						LEFT JOIN froogle ON ALSTYLES.id = froogle.id
						LEFT JOIN datafeeds_style_descriptions 
						  ON ALSTYLES.id = datafeeds_style_descriptions.STYLES_id
					WHERE ALSTYLES.id IN ('7553','7886','8099','8467','8489','8490','8572','8600','8602','8630','8693','8710','8711','8725','8726')";
//					WHERE ALSTYLES.id >".$id." AND 
...
//$log = new phpbmsLog($bleep_statement, "DEBUG STYLE");
		...
		if(!$bleep_result){
			$error= new appError(0,"Could Not Retrieve Styles from Bleep Database","Bleep Import Styles");
		}
...
//$log = new phpbmsLog("style 5=".$mycategoryid, "DEBUG STYLE");
			...
			$queryresult = $this->db->query($querystatement);
			if(!$queryresult){
				$error= new appError(0,"Could Not Retrieve Styles","Bleep Import Styles");
			}
...
//
						...
						case "webenabled":
                                                        //A style is webenable by virtue of being present in the
                                                        // bleep STYLES table.
...
//$log = new phpbmsLog("style 7", "DEBUG STYLE");
				...
				if(!count($styleVerify)){//check for errors
					if($changed){
						...
						$variables["bleep_lastproduct"] = "0000-00-00 00:00:00";
...
//				}
				$variables["packagesperitem"] = ((int) 1);
                                $variables["inactive"]=1;//Set the style as inactive until we are able to validate
				$variables["webenabled"] = 0;//The webenabled is a bleep field, treat separate to inactive
				//if($bleep_record["web_enabled"]==1) $variables["webenabled"] = 1;
				$variables["translationrequired"] = sqlTimeFromString(timeToString(time()));
...
}//end class --importBleepStyles--

if(!isset($noOutput) && isset($db)){

    $clean = new importBleepStyles($db);
    $clean->importBleepStyleRecords();
