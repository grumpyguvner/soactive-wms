<?php
	...
	include("include/styles_products_class.php");

	if(isset($_GET["refid"])) $_GET["id"]=$_GET["refid"];
	if(!isset($_GET["id"])) $error = new appError(300,"Passed variable not set (id)");

	$styleProduct = new styleProduct($db, $_GET["id"]);

	$pageTitle = $styleProduct->getPageTitle();

	// styles table definition uuid
	$reftableid = "tbld:7ecb8e4e-8301-11df-b557-00238b586e42";

  	$whereclause = "
		styleid = '".$styleProduct->styleuuid."'";

	$backurl="../activewms/styles_products.php";
	$base="../../";

	$displayTable= new displaySearchTable($db);
	$displayTable->base = $base;
	$displayTable->initialize("tbld:85867a3d-df59-ed27-4830-370fd5a1493b");
...
//				include_once("modules/activewms/include/styleproducttorecord.php");
	...
	if($displayTable->querytype!="new" and $displayTable->querytype!="edit") {
		...
		$phpbms->cssIncludes[] = "pages/activewms/styleproducts.css";
		$phpbms->jsIncludes[] = "common/javascript/queryfunctions.js";
		$phpbms->topJS[] = 'xtraParamaters="backurl="+encodeURIComponent("'.$backurl.'")+String.fromCharCode(38)+"tabledefid="+encodeURIComponent("'.$reftableid.'")+String.fromCharCode(38)+"refid="+encodeURIComponent("'.$styleProduct->styleid.'");';
