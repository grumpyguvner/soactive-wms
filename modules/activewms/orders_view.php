<?php
	...
	$thetable = new orders($db,NULL,$backurl);
	$therecord = $thetable->processAddEditPage();

	if($thetable->lineitems === NULL)
		$thetable->lineitems = new lineitems($db, $therecord["id"], $therecord["type"]);
	else
		$thetable->lineitems->ordertype = $therecord["type"];

	$phpbms->cssIncludes[] = "pages/order.css";
	$phpbms->jsIncludes[] = "modules/bms/javascript/order.js";
	$phpbms->jsIncludes[] = "modules/bms/javascript/orderaddress.js";
	...
	if(isset($therecord["phpbmsStatus"]))
		...
		$checkDisabled = false;
		if($therecord["type"] == "VOID" || $therecord["type"] == "order")
			$checkDisabled = true;

		$theinput = new inputCheckbox("iscreditmemo", $therecord["iscreditmemo"], "credit memo", $checkDisabled);
		$theform->addField($theinput);

		$theinput = new inputField("cmid", $therecord["cmid"], "related order id", false, NULL, 11);
		        $theinput->setAttribute("readonly", "readonly");
                $theinput->setAttribute("class", "uneditable");
		$theform->addField($theinput);
...
<?php $phpbms->showTabs("orders entry","tab:20276b44-9cfa-403e-4c2a-ac6f0987ae20",$therecord["id"]);?><div class="bodyline">
	<div id="topButtons">
		  <?php if($therecord["id"]){
		  		?><div id="printButton">
	...
	<div id="fsAttributes">
		<fieldset >
			...
			<div>
				...
				<p>
					<?php  if($therecord["type"]=="VOID" || $therecord["type"]=="order") {?>
...
<fieldset id="lineItemsFS">
	...
	<table border="0" cellpadding="0" cellspacing="0" id="LITable">
		...
		<tbody>
		<?php if($therecord["type"]!="order" && $therecord["type"]!="VOID"){?>
		<tr id="LIAdd">
			...
			<div id="vContent4" class="vContent">
				<fieldset>
					...
					<div id="receivableinfo">
						...
						<p class="notes">
							Payments made to accounts receivable orders are done through
		...
		<td colspan="2" class="orderTotalLabels vTabs" id="vTab1"><div>discount<input type="hidden" id="totalBD" name="totalBD" value="<?php echo $therecord["totaltni"]+$therecord["discountamount"]?>" /></div></td>
		...
		<td colspan="2" class="orderTotalLabels"><div>subtotal</div></td>
	...
	<tr>
		<td colspan="2" class="orderTotalLabels vTabs" id="vTab2"><div>tax</div></td>
	...
	<tr>
		<td colspan="2" class="orderTotalLabels vTabs" id="vTab3"><div>shipping</div></td>
	...
	<tr id="totalDisplayTr">
		<td colspan="2" class="orderTotalLabels important"><div>total</div></td>
		<td class="totalItems">
	...
	<tr>
		<td colspan="4" class="orderTotalLabels" id="totalSpacer"><div>&nbsp;</div></td>
	</tr>
	<tr>
		<td colspan="2" class="orderTotalLabels vTabs" id="vTab4"><div>payment</div></td>
	...
	<tr id="lastTotalsTr">
		<td colspan="2" class="orderTotalLabels" nowrap="nowrap"><div>amount due</div></td>
...
<fieldset id="fsInstructions">
	...
	<p>
		...
		<span class="notes"><em>(will not print on order)</em></span><br />
