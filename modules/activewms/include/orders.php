...
if(class_exists("phpbmsTable")){
	class orders extends phpbmsTable{
		...
		function orders($db, $tabledefid = "tbld:3c9b1ba3-0eb4-98b6-7f63-020faa96aa19", $backUrl = NULL){
		...
		function updateStatus($orderid,$statusid,$statusdate,$assignedtoid, $replace = false){

			$orderuuid = getUuid($this->db, $this->uuid, (int) $orderid);
			...
			$querystatement = "
				INSERT INTO
					`orderstatushistory`
					(orderid,orderstatusid,statusdate,assignedtoid) VALUES (";
			$querystatement .= "'".$orderuuid."', ";
		...
		function getDefaultStatus(){
			...
			$querystatement="SELECT uuid FROM orderstatuses WHERE orderdefault=1";
			$queryresult=$this->db->query($querystatement);
		...
		function getStatuses($statusid){
			...
			$querystatement="
				SELECT
					`orderstatuses`.`uuid`,
					`orderstatuses`.`name`,
					`orderstatuses`.`orderdefault`,
					`orderstatuses`.`setreadytopost`,
					`orderstatuses`.`defaultassignedtoid`,
					`users`.`firstname`,
					`users`.`lastname`
				FROM
					(`orderstatuses` LEFT JOIN `users` ON `orderstatuses`.`defaultassignedtoid`=`users`.`uuid`)
				WHERE
					`orderstatuses`.`inactive`='0'
					OR
					`orderstatuses`.`uuid`='".mysql_real_escape_string($statusid)."'
				ORDER BY
					`orderstatuses`.`priority`,
					`orderstatuses`.`name`";
		 ...
		 *
		 * @param string $uuid The uuid of the order record whose credit
		...
		function getDefaults(){
			...
			$therecord["printedinstructions"] = order_DEFAULT_PRINTINSTRUC;
		...
		function verifyVariables($variables){
			...
			if(isset($variables["statusid"])){
				...
				if($this->_availableStatusUUIDs === NULL)
					$this->_availableStatusUUIDs = $this->_loadUUIDList("orderstatuses");
		...
		function prepareVariables($variables){
			...
			if($variables["type"]=="order" && $variables["orderdate"] == "")
				$variables["orderdate"] = dateToString(mktime());
		...
		function addressUpdate($variables, $orderid, $modifiedby, $method){
		// Updates/Inserts address records (billing or shipping )for the sales order's corresponding client
			...
			switch($variables[$idprefix."saveoptions"]){
				...
				case "createAddress":
					...
					$atrRecord["notes"] = "Created from sales order #".$orderid;
		...
		function updateRecord($variables, $modifiedby = NULL, $useUuid = false){
			...
			//can't modify an order
			if(isset($variables["oldType"]))
				if($variables["oldType"] == "order")
					return false;
			...
			if(!hasRights("role:de7e6679-8bb2-29ee-4883-2fcd756fb120")){
				...
				if(ENCRYPT_PAYMENT_FIELDS && (isset($variables["ccnumber"]) || isset($variables["ccexpiration"]) || isset($variables["ccverification"]) || isset($variables["accountnumber"]) || isset($variables["routingnumber"])) ){
					...
					$querystatement .= $fieldlist." WHERE `type` != 'order' AND `type` != 'VOID' AND ".$whereclause;
			...
			if(parent::updateRecord($variables, $modifiedby, $useUuid)){
				...
				if($variables["lineitemschanged"]==1){
					...
					if($this->lineitems === NULL)
						...
						$this->lineitems->orderid = $variables["id"];
		 ...
		 * optionally, the keys "type" (relating to the order type, e.g. "Quote"),
		 * "startdate", and "enddate" which put an upper and lower bound to the
		 * order's datetime (with sql format).
	...
	class lineitems{
		...
		function lineitems($db, $orderid, $ordertype = "Order"){
		...
		function show($lineitemArray){
			...
			foreach($lineitemArray as $therecord){
				...
				?><tr id="li<?php echo $count?>" class="lineitems">
					...
					<td colspan="2" class="lineitemsLeft" <?php if($this->ordertype == "Void" || $this->ordertype == "order") echo 'nowrap="nowrap"'?>>
...
if(class_exists("searchFunctions")){
	class ordersSearchFunctions extends searchFunctions{

		function email_order($useUuid = false){
				...
				require_once("report/report_class.php");
                                include("modules/bms/report/orders_pdf_class.php");

				$processed = 0;

				foreach($this->idsArray as $id){

					$report = new orderPDF($this->db, 'rpt:44b21461-6e67-c284-0ccf-36ab1af47c9b', 'tbld:62fe599d-c18f-3674-9e54-b62c2d6b1883', 'P', 'in', 'Letter');

					if(!$useUuid)
						$report->generate("orders.id = ".$id);
					else
						$report->generate("orders.uuid = ".$id);
				...
				$message = $processed." of ".$count." order PDF";
		...
		function email_quote($useUuid = false){
				...
				require_once("report/report_class.php");
				include("modules/bms/report/orders_pdf_class.php");
				include("modules/bms/report/orders_pdfquote.php");
		...
		function _mark_as_status($statusid, $useUuid = false){
			...
			$whereclause = "(".$whereclause.") AND orders.type!='order' AND orders.type!='VOID' AND orders.statusid !='".$statusid."'";
			...
			$querystatement = "
				SELECT
					...
					orders.orderdate
				FROM
					orders LEFT JOIN paymentmethods ON orders.paymentmethodid = paymentmethods.uuid
				WHERE
					".$whereclause;
			...
			while($therecord = $this->db->fetchArray($queryresult)){
				...
				if($readytopost){
					...
					if(!$therecord["orderdate"] || $therecord["orderdate"] == "0000-00-00")
						$updatestatement.="orderdate = NOW(), ";
				...
				$querystatement = "DELETE FROM orderstatushistory WHERE orderid='".$therecord["uuid"]."' AND orderstatusid='".$statusid."'";
				$deleteresult = $this->db->query($querystatement);

				//insert new history
				$querystatement="INSERT INTO orderstatushistory (orderid,orderstatusid,statusdate,assignedtoid) values (";
		...
		function mark_rtp($useUuid = false){
			...
			$whereclause = "(".$whereclause.") AND orders.type='Order'";
		...
		function mark_aspaid($useUuid = false){
			...
			while($therecord = $this->db->fetchArray($queryresult)){
				...
				$updatestatement = "
					UPDATE orders
					SET
						orders.amountpaid=orders.totalti,
						modifiedby=\"".$_SESSION["userinfo"]["id"]."\",
						modifieddate=NOW()
					WHERE
						id=".$therecord["id"];
		...
		function mark_asorder($useUuid = false){
			...
			$ordersPost = new ordersPost($this->db);

			$count = $ordersPost->post($whereclause);

			$message = $this->buildStatusMessage($count);
			$message .= " posted as order";
		...
		function create_credit_memo($useUuid = false){
			...
			$queryresult = $this->db->query($orderstatusQuery);
			...
			$orders = new phpbmsTable($this->db, "tbld:62fe599d-c18f-3674-9e54-b62c2d6b1883");
			$count = 0;
			if(count($this->idsArray))
				foreach($this->idsArray as $id){

					$orderRecord = $orders->getRecord($id, $useUuid);

					if($orderRecord["type"] == "order" && !$orderRecord["iscreditmemo"]){

						$count++;

						$fieldsList = "";
						$insertValues = "";
						foreach($orderRecord as $name => $value){
...
function defineordersPost(){

	class ordersPost extends tablePost{

		var $maintable = "orders";
		var $datefieldname = "orderdate";
		var $notpostedCriteria = "(orders.type != 'order' && orders.type != 'VOID')";
		...
		function prepareWhere($whereclause=NULL){
			...
			$this->whereclause .= "orders.type='Order' AND readytopost = 1";
		...
		function post($whereclause = NULL, $postsessionid = NULL){
			...
			if(ENCRYPT_PAYMENT_FIELDS){
				$paymentfields = "
					".$this->db->decrypt("`orders`.`ccnumber`")." AS `ccnumber`,
					".$this->db->decrypt("`orders`.`ccexpiration`")." AS `ccexpiration`,
					".$this->db->decrypt("`orders`.`ccverification`")." AS `ccverification`,
					".$this->db->decrypt("`orders`.`routingnumber`")." AS `routingnumber`,
					".$this->db->decrypt("`orders`.`accountnumber`")." AS `accountnumber`";
			}else{
				$paymentfields = "
					orders.ccnumber,
					orders.ccexpiration,
					orders.ccverification,
					orders.routingnumber,
			...
			while($therecord = $this->db->fetchArray($queryresult)){
				...
				if(!$therecord["orderdate"] || $therecord["orderdate"] == "0000-00-00"){
					$therecord["orderdate"] = dateToString(mktime(0,0,0),"SQL");
				...
				if($therecord["type"] == "receivable") {
					...
					$arrecord["itemdate"] = dateToString(stringToDate($therecord["orderdate"],"SQL") );
