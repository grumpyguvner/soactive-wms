...
//print_r("<br/>---curl session---<br/>");
//print_r($session);
//print_r("<br/>---curl options---<br/>");
//print_r($curl_options);
//print_r("<br/>---curl response---<br/>");
//print_r($response);
//die("<br/>---end curl---<br/>");
...
function XML2Array($xml, $name)
{
    ...
    while($xml->read())
    {
        ...
        else if($xml->nodeType == XMLReader::ELEMENT)
        {
            ...
            {
                $childs = self::XML2Array($xml, $node['tag']);
                $node['childs'] = $childs;
            }
...
{
    print_r("ORDER ID: ".$order->getElementsByTagName('id')->item(0)->nodeValue."<br/>");
    print_r("STATUS: ".$order->getElementsByTagName('current_state')->item(0)->nodeValue."<br/>");
    $orderItems = $order->getElementsByTagName('order_row');
    foreach($orderItems as $orderItem)
    {
        print_r("ID: ".$orderItem->getElementsByTagName('id')->item(0)->nodeValue."<br/>");
        print_r("Name: ".$orderItem->getElementsByTagName('product_name')->item(0)->nodeValue."<br/>");
        print_r("Quantity: ".$orderItem->getElementsByTagName('product_quantity')->item(0)->nodeValue."<br/>");
        print_r("Cancelled: ".$orderItem->getElementsByTagName('product_quantity_refunded')->item(0)->nodeValue."<br/>");
    }
}
return false;
...
{
    $node = $reader->expand();
    print_r("<br/>NEXT NODE:<br/>");
    print_r($node->nodeName."=".$node->textContent);
    print_r("<br/>");
}
$reader->close();
...
//$xml = new XMLReader();
//$xml->open($url);
//$assoc = self::XML2Array($xml, "order");
//$xml->close();
//return $assoc;
return false;
